...
class CosmicRaysCleaning:
    def __init__(self, list_of_files, verbose=False):
        self.set_verbose(verbose)
        self.main(list_of_files)
        return
    ...
    @staticmethod
    def remove_cosmic_rays(data, header, prefix):
...
class cosmicsimage:
    def __init__(self, rawarray, pssl=0.0, gain=2.2, readnoise=10.0,
        ...
        self.mask = np.cast['bool'](
            np.zeros(self.rawarray.shape))  # All False, no cosmics yet
    ...
    def clean(self, mask=None, verbose=None):
        ...
        padarray[2:w + 2,
        2:h + 2] = self.cleanarray.copy()  # that copy is important, we need 2 independent arrays
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Remove cosmic rays using LACosmic algorithm."
    )
    ...
    CosmicRaysCleaning(pargs.files, verbose=not pargs.quiet)
