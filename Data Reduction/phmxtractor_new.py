...
def find_local_minimum(x, y, show=False):
    """
    Find the first local minimum using a window that "walks" thought the
    data.
    """
    i = 0
    w = 9
    ...
    i0 = max(0, i - w // 2)
    iz = min(i + w // 2 + 1, len(x))

    slice_index = numpy.arange(0, w) - w // 2
    index = - w // 2
    while index != 0:
        slice_index = slice_index[
            i + slice_index >= 0]  # Protect right border
        slice_index = slice_index[
            i + slice_index <= len(x)]  # Protect left border

        yy = y[i + slice_index]
        index = numpy.argmin(yy) - w // 2
...
class PhaseMap:
    ...
    def get_fwhm(self):
        ...
        if self.show:
            ...
            pyplot.axvline(pars[1] + fwhm_measured / 2, ls='--', c='red', lw=2)
            pyplot.axvline(pars[1] - fwhm_measured / 2, ls='--', c='red', lw=2,
...
class PhaseMapFP(PhaseMap):
    ...
    def get_free_spectral_range(self):
        ...
        # argmin = numpy.argmin(s(z))
        argmin = find_local_minimum(z, s(z), self.show)
        fsr = z[argmin] - self.z[0]
